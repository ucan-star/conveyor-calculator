...
# --- 2. 網頁介面設計 (Streamlit 版本) ---

# 設定網頁標題
st.set_page_config(page_title="輸送機計算系統", page_icon="⚙️", layout="centered")
st.title("⚙️ 進階輸送機工程計算系統")
st.markdown("這是一個專為工程師設計的雲端計算機，支援手機自動排版。")

# 利用 st.form 來包裝所有的輸入欄位與按鈕
with st.form("calculator_form"):
    
    st.subheader("📦 負載與機械結構")
    # Streamlit 的欄位可以排在同一列，我們將 4 個欄位分成兩欄並排
...
if submitted:
    try:
        ...
        st.success("計算成功！結果如下：")
        
        # 使用 Streamlit 內建超美的 metric 元件來顯示數據
        ...
        res_col3.metric("總驅動力", f"{f_total:.2f} N")
        res_col4.metric("馬達輸入扭力", f"{t_motor:.2f} N·m")
        res_col5.metric("理論所需功率", f"{kw:.2f} kW")
